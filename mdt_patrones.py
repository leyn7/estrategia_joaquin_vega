...
def _doble_techo_impulso(df, idx_ruptura, dilatacion, extremo, direction, detalles):
    ...
    agresiva: se entra al toque de la zona del 61.8 (retroceso del impulso, misma
    convención que Entrada Profunda: 61.8-80.9 medido desde el extremo).
    ...
    detalles["dilatacion_maxima"] = dilatacion
    ext = extremo
    if direction == "SELL":  # Doble Techo: el nuevo impulso es bajista
        for i in range(idx_ruptura + 1, len(df)):
            if df.loc[i, 'high'] >= dilatacion:
                return {"estado": "ROTO_POR_RETESTEO_DILATACION",
                        "mensaje": f"Retesteó la máxima dilatación ({dilatacion:.2f}): patrón roto, nuevo análisis.",
                        "detalles": detalles, "idx_muerte": i}
            if df.loc[i, 'low'] < ext:
                ext = df.loc[i, 'low']
            imp = dilatacion - ext
            n618 = ext + imp * 0.618
            if df.loc[i, 'high'] >= n618:
                detalles.update({"impulso_dt": imp, "extremo_impulso": ext,
                                 "entrada_dt_618": n618, "limite_gestion_809": ext + imp * 0.809,
                                 "stop_loss": dilatacion, "hora_gatillo": df.loc[i, 'open_time']})
                return {"estado": "DT_IMPULSO_GATILLO",
                        "mensaje": f"Doble Techo con Impulso: toque del 61.8 en {n618:.2f}. VENTA a mercado. SL {dilatacion:.2f}",
                        "detalles": detalles}
        imp = dilatacion - ext
        n618 = ext + imp * 0.618
        detalles.update({"impulso_dt": imp, "extremo_impulso": ext,
                         "entrada_dt_618": n618, "limite_gestion_809": ext + imp * 0.809,
                         "stop_loss": dilatacion})
        return {"estado": "DT_IMPULSO_ESPERANDO",
                "mensaje": f"Doble Techo con Impulso. Esperando retroceso al 61.8 ({n618:.2f}, "
                           f"zona hasta {ext + imp * 0.809:.2f}). SL {dilatacion:.2f}",
                "detalles": detalles}
    else:  # BUY: Doble Suelo, el nuevo impulso es alcista
        for i in range(idx_ruptura + 1, len(df)):
            if df.loc[i, 'low'] <= dilatacion:
                return {"estado": "ROTO_POR_RETESTEO_DILATACION",
                        "mensaje": f"Retesteó la máxima dilatación ({dilatacion:.2f}): patrón roto, nuevo análisis.",
                        "detalles": detalles, "idx_muerte": i}
            if df.loc[i, 'high'] > ext:
                ext = df.loc[i, 'high']
            imp = ext - dilatacion
            n618 = ext - imp * 0.618
            if df.loc[i, 'low'] <= n618:
                detalles.update({"impulso_dt": imp, "extremo_impulso": ext,
                                 "entrada_dt_618": n618, "limite_gestion_809": ext - imp * 0.809,
                                 "stop_loss": dilatacion, "hora_gatillo": df.loc[i, 'open_time']})
                return {"estado": "DT_IMPULSO_GATILLO",
                        "mensaje": f"Doble Suelo con Impulso: toque del 61.8 en {n618:.2f}. COMPRA a mercado. SL {dilatacion:.2f}",
                        "detalles": detalles}
        imp = ext - dilatacion
        n618 = ext - imp * 0.618
        detalles.update({"impulso_dt": imp, "extremo_impulso": ext,
                         ...
                         "stop_loss": dilatacion})
        return {"estado": "DT_IMPULSO_ESPERANDO",
                "mensaje": f"Doble Suelo con Impulso. Esperando retroceso al 61.8 ({n618:.2f}, "
                           f"zona hasta {ext - imp * 0.809:.2f}). SL {dilatacion:.2f}",
                "detalles": detalles}
...
def evaluate_peak_as_p1(df, p1_idx, zona_max, zona_min, direction):
    ...
    if p2_locked:
        ...
        if confirmada:
            if direction == "SELL":
                confirmada = all(df.loc[k, 'low'] >= p2_val for k in (p2_idx - 2, p2_idx - 1))
            else:
                confirmada = all(df.loc[k, 'high'] <= p2_val for k in (p2_idx - 2, p2_idx - 1))
        if not confirmada:
            return {"estado": "ANULADO_VUELTA_EN_V",
                    "mensaje": f"Vuelta en V: retornó a P1 ({p1_val:.2f}) sin P2 confirmada 2+2. "
                               "Solo 2 pautas: sin información objetiva, se descarta.",
                    "detalles": {"pauta1_price": p1_val, "pauta2_price": p2_val,
        ...
        salio_de_zona = (p2_val < zona_min) if direction == "SELL" else (p2_val > zona_max)
        if not salio_de_zona:
            return {"estado": "ANULADO_SIN_SALIDA_DE_ZONA",
                    "mensaje": f"El rechazo (P2 {p2_val:.2f}) nunca salió de la zona "
        ...
        if direction == "SELL":
            if not toco_1618:
                if h >= fibo_1618:
                    ...
                    # la altura de la P2, muta a Doble Techo con Impulso.
                    if carencia_idx is None and pico_engano < fibo_1382 and l < p2_val:
                        ...
                        if tercio_ext <= p2_val - (p1_val - p2_val) / 3.0:
                            return _doble_techo_impulso(df, j, pico_engano, tercio_ext, direction, detalles)
                ...
                if h >= pico_engano:
                    if confirmado:
                        ...
                        return {"estado": "ROTO_POR_DOBLE_TOQUE",
                                "mensaje": f"Retesteó el extremo confirmado del engaño ({pico_engano:.2f})",
            ...
            if not toco_1618:
                if l <= fibo_1618:
                    toco_1618 = True; pico_engano = l; idx_pico = j
                    tercio_ext = None
                    if carencia_idx is not None:
                        carencia_idx = None
                        calidad = "BUENA (Engaño fraccionado completado)"
                        detalles["calidad"] = calidad
                    if h >= p1_val: gatillo = True; idx_gatillo = j; break
                else:
                    if l < pico_engano:
                        pico_engano = l; idx_pico = j
                    if carencia_idx is None and h >= p1_val and pico_engano <= fibo_1382:
                        calidad = "DEBIL (Carencia)"
                        detalles["calidad"] = calidad
                        carencia_idx = j
                    # Secc 18: la P3 nunca llegó al 138.2 y el precio rompe la P2 en contra
                    if carencia_idx is None and pico_engano > fibo_1382 and h > p2_val:
                        if tercio_ext is None or h > tercio_ext:
                            tercio_ext = h
                        if tercio_ext >= p2_val + (p2_val - p1_val) / 3.0:
                            return _doble_techo_impulso(df, j, pico_engano, tercio_ext, direction, detalles)
    ...
    if gatillo:
        ...
        for k in range(idx_gatillo + 1, len(df)):
            if direction == "SELL" and df.loc[k, 'high'] >= pico_engano:
                estado = "ROTO_POR_STOP_LOSS" if df.loc[k, 'high'] > pico_engano else "ROTO_POR_DOBLE_TOQUE"
                detalles["idx_pico_engano"] = idx_pico
            ...
            if direction == "BUY" and df.loc[k, 'low'] <= pico_engano:
                estado = "ROTO_POR_STOP_LOSS" if df.loc[k, 'low'] < pico_engano else "ROTO_POR_DOBLE_TOQUE"
                detalles["idx_pico_engano"] = idx_pico
                return {"estado": estado, "mensaje": "El precio volvió al extremo del engaño tras el gatillo.",
                        "detalles": detalles, "idx_muerte": k, "idx_pico_engano": idx_pico}
