...
- **El Método de las Muñecas Rusas (Buscando el Origen Correcto):** Para encontrar un punto de inicio correcto, empezamos en Mensual. Localizamos el impulso mayor absoluto del gráfico. **El retroceso de este gran fractal 1 se convierte automáticamente en el impulso del fractal 2.** A su vez, el retroceso del fractal 2 será el impulso del fractal 3. Podemos ir quitando impulsos y desgranando el gráfico como muñecas rusas hasta llegar a la temporalidad de 4 Horas.
- **Elección del Origen y Potencia vs Velocidad:** CUALQUIERA de los orígenes de esos fractales anidados es un punto de inicio 100% correcto para mapear el mercado. La elección depende del operador: Cuanto más atrás inicies (fractal mayor), **más potentes y fiables** serán tus zonas, pero el trading será más lento (zonas más grandes). Cuanto más cerca del precio inicies (fractal menor, hasta H4), tendrás entradas más rápidas, pero tus zonas tendrán **menos potencia** institucional. El error clásico es iniciar el mapa lo más cerca posible solo por la "tensión de estar en el mercado".
- **Definición de Fractal:** Un fractal nace cuando un impulso deja un extremo (máximo o mínimo) y el precio retrocede, dejando una secuencia confirmada de **2 velas cerradas a la izquierda y 2 a la derecha** de ese extremo.
- **Validación del Punto de Control (Regla del 1/3):** Un retroceso NO se convierte en un "Punto de Control Válido" hasta que el impulso siguiente logra romper el extremo del fractal por al menos **1/3 de la altura total de ese retroceso**. (Si el precio hace un nuevo extremo más profundo antes de romper, la altura del retroceso crece y el requisito del 1/3 debe recalcularse).
- **Desgrane del Ruido (Evolución de Fractalidad):** "Los movimientos menores dan lugar a movimientos mayores". Cada vez que un nuevo Punto de Control es **validado** (al superar su 1/3), se mira hacia atrás: Si algún Punto de Control válido anterior tiene una altura (grado) MENOR al recién validado, ese punto anterior **MUERE y queda obsoleto**. El retroceso mayor se come al menor.
- **Trazado de Ciclos:** Es un error grave trazar ciclos desde fractales muertos o desde retrocesos que nunca lograron validarse por el 1/3. Esos movimientos son simples "ruidos". Las únicas anclas válidas para trazar ciclos son los Puntos de Control que han sobrevivido al desgrane o el Origen Absoluto.
...
2. **Caso 2 (Búsqueda obligatoria de Sub-ciclos):** El precio tocó el 38.2% (ciclo activo y zonas dibujadas), PERO el precio está flotando sin tocar ninguna zona. Al no haber trabajo en zona, **no tenemos ninguna de las 4 informaciones**. El ciclo macro es 100% correcto y es el que manda, pero actualmente no nos da información operativa. Esto nos obliga inmediatamente a bajar a buscar un Segundo Ciclo (Sub-ciclo) dentro de la estructura actual que SÍ esté tocando una de sus propias zonas para poder operar.
3. **Caso 3 (El Dibujo más "Lotoso" - Sin Ciclo):** El retroceso desde el origen macro NUNCA llegó al 38.2%. **NO CAMBIES EL PUNTO DE ORIGEN.** El protocolo exige 3 pasos estrictos:
   - **Paso 1 (Marcas de Alerta):** Traza 3 líneas horizontales con un "color de alerta": El Origen, el nivel 38.2% (donde se activaría) y la zona del 61.8%. Si el precio toca la alerta del 38.2% en el futuro, el ciclo global cobra vida.
   - **Paso 2 (Buscar en Extremos Anteriores):** Si es bajista, prueba trazar fibos desde mínimos relevantes anteriores para ver si el precio está trabajando alguna "Parte Baja". (Si rompieron su extensión, descártalos).
   - **Paso 3 (Desgranar y Buscar Sub-ciclo Vigente):** Si lo anterior falla, aplica el protocolo de "Desgrane del Ruido" (validando puntos de control con el 1/3) a todo el impulso actual. Localiza el **primer Punto de Control válido sobreviviente** y traza un Fibo desde ahí. Si ese sub-ciclo sí se activó (tocó su 38.2%), ¡ese se convierte temporalmente en el **"ciclo vigente que manda"** en el gráfico!
